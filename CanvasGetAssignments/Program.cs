...
class Program
{
  ...
  static async Task Main(string[] args)
  {
    ...
    Enrollment[] allEnrollments = await builder.GetAllEnrollments();

    Console.WriteLine();
    foreach (Course course in currentCourses)
    {
      foreach (Enrollment enrollment in allEnrollments)
      {
        if (enrollment.CourseId == course.Id)
        {
          Console.WriteLine($"Matched course {course.Name}");
    ...
    {
      string suffix = "";
      if (course.Enrollment.Grades != null)
      {
        suffix = $" ({course.Enrollment.Grades.CurrentScore.ToString()}%)";
      }

      // Add a header for each course
      sb.AppendLine($"#### [{course.Name + suffix}]({Url.Combine(course.HtmlUrl, "grades")})");
      sb.AppendLine();
