...
namespace CanvasApi
{
  public class AssignmentBuilder
  {
    ...
    {
      string? userJson = await _caller.Call($"users/self");
      User user = JsonSerializer.Deserialize<User>(userJson) ?? new();
      return user;
    }

    public async Task<Enrollment[]> GetAllEnrollments()
    {
      User user = await GetUser();
      string? enrollmentJson = await _caller.Call($"users/{user.Id}/enrollments");
      Enrollment[] enrollments = _ParseJson<Enrollment>(enrollmentJson);
      foreach (var e in enrollments)
      {
        if (e.Type != "StudentEnrollment") { continue; }
        Console.WriteLine(e.JsonContent);
        var d = JsonSerializer.Deserialize<Dictionary<string, object>>(e.JsonContent);
        e.Grades = JsonSerializer.Deserialize<Grade>((JsonElement)d["grades"]);
      }
      return enrollments;
    }
  }
